...
"4-6":{핵심:"다윗은 헤브론에서 유다의 왕이 되고 긴 내전이 이어지지만 다윗의 집은 점점 강해집니다.",성품:"하나님은 인내하며 기다리는 자와 함께하시는 분입니다.",묵상:"내가 기다리고 있는 하나님의 약속이 있나요? 하나님의 때는 결코 늦지 않습니다.",기도:"때를 아시는 하나님, 내 성급함을 내려놓고 당신의 때를 기다리는 믿음을 허락하소서.",구절:"만군의 하나님 여호께서 함께 계시니 다윗이 점점 강성하여 가니라 (사무엘하 5:10)"},
    ...
    <div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",padding:20}} onClick={()=>setShowShareModal(false)}>
          <div style={{background:"#12151F",border:`1px solid ${theme.border}`,borderRadius:20,padding:"24px",maxWidth:400,width:"100%"}} onClick={e=>e.stopPropagation()}>
            ...
            <textarea ref={shareTextRef} readOnly value={shareText}
              ...
              style={{width:"100%",background:"rgba(255,255,255,.04)",border:`1px solid ${theme.border}`,borderRadius:12,padding:"14px",color:"#EDE5D5",fontSize:13,lineHeight:1.8,height:160,resize:"none"}}/>
            <div style={{display:"flex",gap:8,marginTop:12}}>
              ...
              <button onClick={()=>setShowShareModal(false)} style={{background:"rgba(255,255,255,.05)",border:"1px solid rgba(255,255,255,.1)",borderRadius:12,padding:"12px 16px",color:"#6A5E50"}}>닫기</button>
            </div>
          </div>
        </div>
      )}
      ...
      {recState==="idle" && (
        <div style={{textAlign:"center",padding:"8px 0"}}>
          <button onClick={startRecording} style={{background:`linear-gradient(135deg,${theme.color},${theme.color}CC)`,border:"none",borderRadius:100,padding:"12px 28px",color:"#08090F",fontSize:14,fontWeight:600}}>🎙 녹음 시작</button>
        </div>
      )}
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#06091A 0%,#080C1E 100%)",color:"#E8E0D0",fontFamily:"'Noto Serif KR',serif"}}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        .fade{animation:fadeUp .6s ease both}
        @keyframes fadeUp{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}
        .btn{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.09);color:#9A8E7A;padding:9px 16px;border-radius:8px;cursor:pointer;font-size:13px}
      `}</style>
      ...
      <div style={{maxWidth:660,margin:"0 auto",padding:"0 20px"}}>
        <header style={{display:"flex",justifyContent:"space-between",padding:"22px 0 0"}}>
          <div>
            <div style={{fontSize:10,letterSpacing:".2em",color:"#3A3028"}}>BASIC Community Church</div>
            <div style={{fontSize:12,color:"#6A5E50"}}>{fmtLong(viewDate)}</div>
          </div>
          <button onClick={()=>signOut(auth)} style={{background:"none",border:"1px solid rgba(255,255,255,.08)",borderRadius:20,padding:"4px 10px",color:"#5A5040",fontSize:10}}>로그아웃</button>
        </header>

        <div style={{display:"flex",gap:3,justifyContent:"center",margin:"14px 0 0"}}>
          {TABS.map(([t,l]) => (
            <button key={t} onClick={()=>setTab(t)} style={{color:tab===t?theme.color:"#5A5242",background:tab===t?theme.bg:"none",border:"none",padding:"7px 12px",borderRadius:20,fontSize:11}}>
              {l}
            </button>
          ))}
        </div>
        ...
        {tab==="main" && (
          <div key={key} className="fade">
            <h1 style={{fontSize:60,textAlign:"center",color:theme.color,marginTop:20}}>{theme.name}</h1>
            <div style={{background:`linear-gradient(135deg,${theme.bg},rgba(0,0,0,.06))`,border:`1px solid ${theme.border}`,borderRadius:20,padding:"18px 22px",marginBottom:12}}>
              <div style={{fontSize:26,fontWeight:600,color:"#EDE5D5",marginBottom:4}}>{raw}</div>
              <div style={{fontSize:13,color:theme.color+"88",marginBottom:12}}>{expanded}</div>
              <p style={{fontSize:14,color:"#C0B49A",lineHeight:1.85}}>{devotional}</p>
            </div>
            <div style={{display:"flex",justifyContent:"center",gap:6}}>
              <button className="btn" onClick={()=>nav(-1)}>← 이전</button>
              <button className="btn" onClick={toggleDone}>{done.has(key)?"✓ 완료":"완료 체크"}</button>
              <button className="btn" onClick={()=>nav(1)}>다음 →</button>
            </div>
          </div>
        )}

        {/* ═══ 기도 녹음 탭 (갈라디아서 2:20 고정됨) ═══ */}
        {tab==="voice" && (
          <div key={`voice-${key}`} className="fade" style={{paddingTop:8}}>
            <div style={{background:`linear-gradient(135deg,${theme.bg},rgba(0,0,0,.05))`,border:`1px solid ${theme.border}`,borderRadius:22,padding:"24px 22px",marginBottom:14}}>
              <div style={{textAlign:"center",marginBottom:20}}>
                <div style={{fontSize:10,letterSpacing:".25em",color:theme.color+"77",marginBottom:10}}>오늘의 말씀 선포 및 기도</div>
              ...
              <VoiceRecorder dateKey={key} passageRaw={raw} theme={theme} onSave={()=>{const v=new Set(voiceDone); v.add(key); setVoiceDone(v);}}/>
            </div>
          </div>
        )}

        {/* 달력 등 나머지 탭은 기존 원본 코드의 로직을 따릅니다... */}
      </div>
    </div>
  );
}
