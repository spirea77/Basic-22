...
// 모든 날짜에 '구절' 항목이 추가되었습니다! 
const DEVOTIONALS = {
"3-22":{핵심:"기드온의 아들 아비멜렉이 형제 70명을 죽이고 왕이 되지만, 하나님은 그 교만과 폭력을 결코 묵과하지 않으십니다. 아비멜렉의 3년 통치 끝에 하나님은 그 악을 돌려보내시며 공의를 이루십니다.",성품:"하나님은 공의로우신 분입니다. 인간이 아무리 권력으로 진실을 덮으려 해도, 하나님의 공의는 반드시 이루어집니다. 역사는 하나님의 공의로 움직입니다.",묵상:"내 삶에서 내가 통제하려 하지만 하나님께 맡겨야 할 영역이 있지 않나요? 오늘 내 손에 쥔 것을 하나님의 공의 앞에 내려놓는 하루가 되기를 바랍니다.",기도:"공의로우신 하나님, 내 삶의 모든 영역에서 당신의 주권을 인정하며, 내 야망이 아닌 하나님의 뜻이 이루어지게 하소서.",구절:"하나님이 아비멜렉과 세겜 사람들 사이에 악한 영을 보내시매 세겜 사람들이 아비멜렉을 배반하였으니 (사사기 9:23)"},
      ...
      mr.onstop = () => {
        ...
        reader.onloadend = () => {
          ...
          try {
            ...
            localStorage.setItem(metaKey, JSON.stringify({ duration: dur, savedAt: new Date().toLocaleString("ko-KR"), size: Math.round(blob.size/1024) }));
            setRecState("saved");
            if (onSave) onSave();
          } catch {
    ...
    <div>
      ...
      {recState === "idle" && (
        <div style={{textAlign:"center",padding:"8px 0"}}>
          ...
          <div style={{fontSize:11,color:"#3A3028",marginTop:14}}>최대 5분</div>
...
export default function App() {
  ...
  useEffect(() => {
    try { 
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (k && k.startsWith("bc365_audio_meta_")) {
          vSet.add(k.replace("bc365_audio_meta_", ""));
        }
      }
      setVoiceDone(vSet);
    } catch {}
  ...
  const toggleDone = () => {
    ...
    setDone(next);
    try { localStorage.setItem("bc365v2", JSON.stringify([...next])); } catch {}
  };

  const handleVoiceSaved = () => {
    setVoiceDone(prev => { const n = new Set(prev); n.add(key); return n; });
    if (!done.has(key)) {
      setDone(prev => {
        const n = new Set(prev);
        n.add(key);
        try { localStorage.setItem("bc365v2", JSON.stringify([...n])); } catch {}
        return n;
      });
    }
  };
  ...
  const handleVoiceDeleted = () => {
    setVoiceDone(prev => { const n = new Set(prev); n.delete(key); return n; });
  };
          ...
          <div key={key} className="fade">
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",gap:8,marginBottom:18,flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                {voiceDone.has(key) && (
                  <span style={{fontSize:12,color:theme.color,background:theme.bg,padding:"8px 14px",borderRadius:20,border:`1px solid ${theme.border}`,fontFamily:"'Noto Serif KR',serif"}}>
                    🎙 녹음 완료
                  </span>
                )}
            ...
            <div style={{background:`linear-gradient(135deg,${theme.bg},rgba(0,0,0,.05))`,border:`1px solid ${theme.border}`,borderRadius:22,padding:"26px 24px",marginBottom:16}}>
              ...
              <div style={{textAlign:"center",marginBottom:22}}>
                ...
                {d && d.구절 ? (
                  ...
                  <div style={{fontSize:14,color:"#8A7E6E",lineHeight:1.7,fontWeight:300}}>
                    오늘 묵상한 본문 중 하나님의 성품이 담긴<br/>말씀을 먼저 소리 내어 선포하고 기도해 보세요.
                  </div>
                )}
              </div>
          ...
          <div className="fade" style={{paddingTop:20}}>
            ...
            <div style={{display:"flex",gap:14,justifyContent:"center",margin:"14px 0 8px",flexWrap:"wrap"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:4}}>
                ...
                <span style={{fontSize:11,color:"#6A5E50"}}>묵상 완료</span>
              </div>
              <div style={{display:"flex",alignItems:"center",gap:4}}>
                <span style={{fontSize:10,color:"#C9A84C"}}>🎙</span>
                <span style={{fontSize:11,color:"#6A5E50"}}>녹음 완료</span>
              </div>
            </div>
            <div style={{height:30}}/>
          </div>
